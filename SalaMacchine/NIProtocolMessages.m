...
@implementation NIButtonsChangedEvent

- (NSString *)description;
{
    return [NSString stringWithFormat:@"{%d, %d}", self.buttonId, self.on];
}

@end

@implementation NIButtonsChangedMessage
...
+ (NIMessage *)messageFromData:(NSData *)data;
{
    ...
    {
        NIButtonsChangedEvent * e = [NIButtonsChangedEvent new];
        e.buttonId = eventsList[i * 2];
        e.on       = ((uint32_t *)eventsList)[i * 2 + 1];
        [events addObject:e];
    }
